...
#IFNDEF EXP_OUT_ENABLE_4L7D

#define EXP_OUT_ENABLE_4L7D      PIN_B0   //ST_CP  = RCK_?
...
void write_expanded_outputs_4L7D(BYTE* eo) {
  BYTE i;

  output_low(EXP_OUT_CLOCK);
  output_low(EXP_OUT_ENABLE_4L7D);

  for(i=1;i<=NUMBER_OF_74595_4L7D*8;i++) {  // Clock out bits from the eo array
    if((*(eo+(NUMBER_OF_74595_4L7D-1))&0x80)==0)
      output_low(EXP_OUT_DO);
    else
      output_high(EXP_OUT_DO);
   shift_left(eo,NUMBER_OF_74595_4L7D,0);
   output_high(EXP_OUT_CLOCK);
   output_low(EXP_OUT_CLOCK);
  }
  output_high(EXP_OUT_ENABLE_4L7D);
  output_low(EXP_OUT_ENABLE_4L7D);
